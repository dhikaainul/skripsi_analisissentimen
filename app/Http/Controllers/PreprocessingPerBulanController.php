...
class PreprocessingPerBulanController extends Controller
{
    use KelolaDataTrait; // memanggil trait

    public function datapreprocessingperbulan()
    {
        $preprocessing = PreprocessingPerBulan::all();
        // dd($preprocessing);
        return view('content.preprocessingperbulan', ['preprocessing' => $preprocessing]);
    }
    public function preprocessing()
    {
        DB::beginTransaction();
        try {
            $dataset = DatasetPerBulan::all(); //menggabungkan keseluruhan function
            ...
            PreprocessingPerBulan::query()->truncate();  //Hapus semua data preprocesiing
            // dd($data2);
            foreach ($stopword_removal as $key => $value) {
                // dd($value['text']);
                PreprocessingPerBulan::create([
                    'dataset_id' => $value['id'],
                    'dataset_author' => $value['author'],
                    'hasil_preprocessing_data' => $value['text'],
                    'dataset_month' =>$value['month'],
                    'dataset_date' =>$value['date'],
                ]); // Insert ke database
            }
            ...
            return redirect()->route('preprocessingperbulan')->with('sukses', 'Dataset Berhasil Di Preprocessing');
        } catch (\Exception $e) {
